...
if __name__ == '__main__':
    ...
    parser.add_argument("--image_root", default='./data/', type=str)
    ...
    # Initialize 2D Gaussian model
    scene_gaussians = GaussianModel(dataset.sh_degree)
    scene = Scene(args, scene_gaussians, load_iteration=-1, shuffle=False)
    ...
    for i, image_path in tqdm(enumerate(sorted(os.listdir(os.path.join(dataset.source_path, 'images'))))):
        ...
        masks = torch.load(os.path.join(os.path.join(dataset.source_path, 'sam_masks'), 
    ...
    background = torch.zeros(scene_gaussians.get_xyz.shape[0], 3, device='cuda')

    for it, view in tqdm(enumerate(cameras)):
        # Use 2D Gaussian specific renderer
